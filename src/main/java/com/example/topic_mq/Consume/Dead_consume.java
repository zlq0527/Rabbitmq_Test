...
public class Dead_consume {
	...
	public void receive(Message msg1, Channel channel) throws IOException {
		...
		if (!ack) {
			System.out.println("消息接受异常_含有error,准备拒收!");
			//basicNack方法
			// 第二个参数 multiple：是否批量.true:将一次性拒绝所有小于deliveryTag的消息。 false:只拒绝单条消息.
